function jbsalpha(astra_out,rhos,tcrash,tauELM,deltaELM,varargin)
% jbsalpha(astra_out,rhos,tcrash,tauELM,deltaELM,varargin)
...
for ii = 1:size(pe,2)
	...
	% J
	jbs(:,ii) = jbs(:,ii) ./ itot(end,ii);%           [m^-2]
	jbs(:,ii) = jbs(:,ii) .* surface(:,ii);%          [] is ok???
end
% ALPHA
...
min_j     = min(floor(5*min(min(jbs(irhos,:)))),0)/5;
max_j     = ceil(5*max(max(jbs(irhos,:))))/5;
...
for jj = 1:length(irhos)
	...
	toplot_y_1(:,jj) = jbs(irhos(jj),it1:it2-1);
	toplot_x_2(:,jj) = alpha(irhos(jj),it2:it3-1);
	toplot_y_2(:,jj) = jbs(irhos(jj),it2:it3-1);
	toplot_x_3(:,jj) = alpha(irhos(jj),it3:it4-1);
	toplot_y_3(:,jj) = jbs(irhos(jj),it3:it4-1);
	toplot_x_4(:,jj) = alpha(irhos(jj),it4:itend);
	toplot_y_4(:,jj) = jbs(irhos(jj),it4:itend);
end
%keyboard
...
print('-dpsc',['pics/' int2str(shot) '_' num2str(t0) '_jbsalpha' addname '.ps']);
set(gcf,'paperpositionmode','auto');
print('-dpsc',['pics/' int2str(shot) '_' num2str(t0) '_jbsalpha' addname '_ppm.ps'])
%refax = [-0.1 3.6 0 1.4];
%axis(refax);
%print('-dpsc',['pics/' int2str(shot) '_' num2str(t0) '_jbsalpha' addname '_scaledppm.ps'])
