function johmalpha(astra_out,rhos,tcrash,tauELM,deltaELM,varargin)
% johmalpha(astra_out,rhos,tcrash,tauELM,deltaELM,varargin)
...
Bt = astra_out.btor;%                 [T]
iohm = astra_out.iohm .* 1e6;
for ii = 1:size(volum,2)
	[aa johm(:,ii)] = interpos(volum(:,ii),iohm(:,ii));
end
johm = johm .* (2 * pi * 0.88);% [A m^-2]
...
for ii = 1:size(pe,2)
	...
	% J
	johm(:,ii) = johm(:,ii) ./ itot(end,ii);%           [m^-2]
	johm(:,ii) = johm(:,ii) .* surface(:,ii);%          [] is ok???
end
% ALPHA
...
min_j     = min(floor(5*min(min(johm(irhos,:)))),0)/5;
max_j     = ceil(5*max(max(johm(irhos,:))))/5;
...
for jj = 1:length(irhos)
	...
	toplot_y_1(:,jj) = johm(irhos(jj),it1:it2-1);
	toplot_x_2(:,jj) = alpha(irhos(jj),it2:it3-1);
	toplot_y_2(:,jj) = johm(irhos(jj),it2:it3-1);
	toplot_x_3(:,jj) = alpha(irhos(jj),it3:it4-1);
	toplot_y_3(:,jj) = johm(irhos(jj),it3:it4-1);
	toplot_x_4(:,jj) = alpha(irhos(jj),it4:itend);
	toplot_y_4(:,jj) = johm(irhos(jj),it4:itend);
end
%keyboard
...
print('-dpsc',['pics/' int2str(shot) '_' num2str(t0) '_johmalpha' addname '.ps']);
set(gcf,'paperpositionmode','auto');
print('-dpsc',['pics/' int2str(shot) '_' num2str(t0) '_johmalpha' addname '_ppm.ps'])
%refax = [-0.1 3.6 0 1.4];
%axis(refax);
%print('-dpsc',['pics/' int2str(shot) '_' num2str(t0) '_johmalpha' addname '_scaledppm.ps'])
