...
C Emiliano 2005
     ...
     3   rkuth,rklth,fpthrs,ikthrs,sawcoef(1:3),dwth(1:3)
      ...
      save TMIX
      data TMIX/-99999./
      real*8, save :: tsaw0, tsaw1
      integer ii_rho_q1,ii_q1
      double precision s_i_1
      data tsaw0/0./
      istep=istep+1
...
!      if (t.le.1) then
     ...
     1   dwth(1),dwth(2),dwth(3),nres,icrash
      ...
      tsaw1=TIME-tsaw0
      tsaw0=TIME
      ii_rho_q1=MINLOC(ABS(q_i(5:NA1)-1.),1)
      s_i_1=s_i(ii_rho_q1)
      if (tsaw1 .gt. 1.d-4 .and. icrash .eq. 3) then
       write(*,'(a,f4.2,a,f6.4,a,f6.4,a,f6.2,a,f5.3)')
     &   ' CMHD4=',X3,'  q0=',q_i(1),'  s1=',s_i_1,
     & '   delta_t=',tsaw1*1000,'ms t=',TIME
      endif
...
!      stop
	...
	if (LEQ(3).gt.0)  TI(J)=ti_o(J)/1.D3
        MU(J)=1.D0/q_o(J)
        FP(J)=spsi(J)*psi_o(J)
!        write(18,*) a_i(J)/a_i(jmax),ne_i(J)/1.D19,
!     &     ne_o(J)/1.D19,fps_a(J),fps_p(J),q_i(J),q_o(J)
      enddo
      end
